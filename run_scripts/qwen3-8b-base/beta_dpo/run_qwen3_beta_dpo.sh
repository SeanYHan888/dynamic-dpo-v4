...
usage() {
  cat <<'EOF'
Usage: run_script/beta_dpo/run_qwen3_beta_dpo.sh <ultrafeedback|hh-helpful|hh-harmless> [extra run_beta_dpo args...]
...
echo "Starting $VARIANT Beta-DPO run: $RUN"
...
UPLOAD_ARGS=(
  ...
  --checkpoint-dir "$RUN_DIR"
  --repo-id "$HF_REPO_ID"
  --commit-message "Upload validated Beta-DPO checkpoint"
)
