...
@pytest.mark.skipif(
    ...
    reason=(
        "Set EDPO_REFERENCE_PYTHON to a Python interpreter with the original reference-codes/e-dpo stack "
        "(TRL 0.13 compatible) and EDPO_PARITY_MODEL to a shared model checkpoint to run trajectory parity."
    ),
)
def test_short_training_trajectory_parity_against_reference_stack():
